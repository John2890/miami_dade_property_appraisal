...
MODEL_COLUMNS = [
    'living_sqft', 'lot_sqft', 'eff_age', 'num_buildings',
    ...
    'age_Modern', 'age_Mid_Century', 'age_Historic'
]
...
if st.button("💰 Predict Sale Price", use_container_width=True):
    ...
    eff_age = current_year - eff_year_built

    raw_age = current_year - year_built
    age_Modern, age_Mid_Century, age_Historic = 0, 0, 0

    if raw_age > 10 and raw_age <= 40:
        age_Modern = 1 
    elif raw_age > 40 and raw_age <= 75:
        age_Mid_Century = 1
    elif raw_age > 75:
        age_Historic = 1
    ...
    input_data['age_Modern'] = age_Modern
    input_data['age_Mid_Century'] = age_Mid_Century
    input_data['age_Historic'] = age_Historic
    ...
    st.caption(
        ...
        f"with {bedrooms} bed / {total_baths:.1f} bath, built in {year_built} "
        f"{f'(Renovated {eff_year_built})' if is_renovated else ''}, "
...
st.markdown(
    ...
    "Random Forest model · Trained on Florida DOR + Miami-Dade Open Data Hub records"
    "</div>",
